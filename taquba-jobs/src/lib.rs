...
//! multi-step processes: use `taquba-jobs` when the caller awaits a typed
//! return value, and `taquba-workflow` when one entity moves through
//! durable steps with cancellation and a terminal hook. Chaining jobs
//! through [`JobContext::submit`] to model a multi-step process is a sign
//! the work belongs in a workflow. To run one pipeline over many inputs
//! with batch progress and cost rollup, use `taquba-bulk`.
//!
//! # Architecture
//!
