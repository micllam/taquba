...
- Key layout: the format and scan-ordering rationale are documented in the `taquba/src/keys.rs` module docs. When adding a key space, add a `KeyTag` variant with its layout documented on the variant, follow the existing field conventions (the field the scan orders by comes first) and extend the non-overlap test.
...
- Attempt history (`Queue::attempt_history`): the append mechanism and lifetime rule are documented in the `taquba/src/history.rs` module docs. Any new path that removes a job's last record must also delete `attempt_history_key(id)` in the same transaction (nothing else removes histories) and extend the `attempt_history` tests in `taquba/src/queue.rs`.
- Offloaded payloads (`JobRecord::payload_ref`): the ordering invariant and object-naming rationale are documented in the `taquba/src/payload_store.rs` module docs. Any new path that writes or removes a record must preserve the ordering: write the payload object before the record-writing transaction, delete it only after the record-removing transaction commits (best-effort, never before) and extend the offload tests in `taquba/src/queue.rs`.
...
- Keep each crate's `lib.rs` top-level `//!` docstring and its `README.md` in content parity: anything substantive (new sections, design notes, semantics callouts) lands in both. Format may differ (lib.rs uses intra-doc `[Foo]` links and `#`-hidden doctest lines; README uses URL links and full `#[tokio::main]` blocks so code is copy-pasteable). Shields.io badges (version/docs.rs/license) are a deliberate exception: they live in `README.md` only and must never be copied into `lib.rs`, where a version or docs badge would be circular (the `//!` is the docs.rs page) and render poorly.
